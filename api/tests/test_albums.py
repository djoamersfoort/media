...
async def test_create_album_admin(admin_client):
    ...
    with patch("os.mkdir"):
        response = await admin_client.post(
            "/albums", json=album_data, headers={"Authorization": "Bearer mock_token"}
        )
...
async def test_get_albums(admin_client, db_session):
    ...
    db_album = models.Album(
        ...
        description="Desc",
        order=0,
        preview_id=db_preview.id,
    )
    db_session.add(db_preview)
    db_session.add(db_album)
    await db_session.commit()

    response = await admin_client.get(
        "/albums", headers={"Authorization": "Bearer mock_token"}
    )
    ...
    assert data[0]["id"] == str(album_id)
    assert data[0]["preview"]["id"] == str(db_preview.id)
...
async def test_get_album_details(admin_client, db_session):
    ...
    album_id = uuid4()
    db_album = models.Album(
        id=album_id, name="Detailed Album", description="Detailed Desc", order=0
    )
    db_session.add(db_album)
    db_item = models.Item(
        id=uuid4(),
    ...
    response = await admin_client.get(
        f"/albums/{album_id}", headers={"Authorization": "Bearer mock_token"}
    )
    ...
    assert data["items"][0]["id"] == str(db_item.id)
...
async def test_update_album(admin_client, db_session):
    ...
    album_id = uuid4()
    db_album = models.Album(
        id=album_id, name="Old Name", description="Old Desc", order=0
    )
    db_session.add(db_album)
    await db_session.commit()
...
async def test_delete_album(admin_client, db_session):
    ...
    with patch("os.path.exists", return_value=True), patch("os.rmdir"), patch(
        "app.db.crud.delete_items"
    ):
