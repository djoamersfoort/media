...
async def test_get_albums(admin_client, db_session):
    ...
    album_id = uuid4()
    db_preview = models.Item(id=uuid4(), path="test/preview.jpg", cover_path="test/cover.jpg", album_id=album_id, type=models.Type.IMAGE, width="100", height="100")
    db_album = models.Album(id=album_id, name="Existing Album", description="Desc", order=0, preview_id=db_preview.id)
...
async def test_get_album_details(admin_client, db_session):
    ...
    db_session.add(db_album)
    db_item = models.Item(id=uuid4(), album_id=db_album.id, path='tests/item.jpg', cover_path='tests/cover.jpg', type=models.Type.IMAGE, width='100', height='100')
